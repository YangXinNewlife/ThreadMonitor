...
public class PathChangedTriger extends Thread{
	...
	public PathChangedTriger(File file){
		this.file = file;
		this.lastpath = this.file.getAbsolutePath();
	}
		...
		for(int i=0; i<dirs.length; i++){
			if(!dirs[i].isDirectory()){
				if(flag && files.get(dirs[i].getAbsoluteFile())!= dirs[i].lastModified()){
					System.out.println("File path had been altered---");
					try {
						dt.recordDetail(file, "path-changed");
	...
	public void run() {
		...
		while(true){
			...
			if(!file.isDirectory()){
				if(lastpath != file.getAbsolutePath()){
					System.out.println("File path had been altered---");
					try {
						dt.recordDetail(file, "path-changed");
					} catch (FileNotFoundException e) {
